	...
	function builder_emps_table(result) {
		...
		$.each(emps, function(index, item) {
			...
			var editBtn = $("<button></button>").addClass(
					"btn btn-primary edit_btn").append(
					$("<span></span>").addClass(
							"glyphicon glyphicon-pencil btn-xs")).append("编辑");
			editBtn.attr("edit-id", item.empId);
			var deletBtn = $("<button></button>").addClass(
					"btn btn-danger delete_btn").append(
					$("<span></span>").addClass(
							"glyphicon glyphicon-trash btn-xs")).append("删除");
			deletBtn.attr("delete-id", item.empId);
			var btnTd = $("<td></td>").append(editBtn).append(" ").append(
					deletBtn);

			$("<tr></tr>").append(empIDTd).append(empNameTd).append(genderTd)
					.append(emailTd).append(deptName).append(btnTd).appendTo(
							"#emps_table tbody");
		})
	}
